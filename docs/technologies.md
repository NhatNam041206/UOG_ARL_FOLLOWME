...
| `Pillow` (PIL) | ≥10.0.0 | `scripts/register_person.py`'s Tkinter UI — converts each BGR camera frame to a `PIL.Image` → `ImageTk.PhotoImage` for display in a `tk.Label`. Was a transitive-only dependency before the registration UI. |
| `tkinter` | stdlib | `scripts/register_person.py`'s `RegistrationApp`/`CaptureWindow` — the CRUD UI and live capture preview. Not in `requirements.txt` (ships with the standard Python install). |
...
| `torchreid` | NOT in `requirements.txt` | Used ONCE, offline, on a dev machine (not on-device) to export `modules/autocar/models/osnet_x1_0_msmt17.onnx` (see the `autocar` vendor row below) — KaiyangZhou/deep-person-reid, the reference implementation OSNet's own paper is published through. Not a runtime dependency of any live pipeline (its only in-repo consumer, `modules/appearance_verifier`, was removed 2026-08-27 as dead code — never wired into any pipeline). Deliberately excluded from `requirements.txt`: its setup.py imports the whole package (including runtime deps like scipy/matplotlib/tb-nightly) just to read its version string, which fails under pip's isolated build sandbox — this broke `pip install -r requirements.txt` on a Raspberry Pi. If the export step ever needs re-running, install it by hand in a throwaway venv on a dev machine instead. |
| `gdown` | NOT in `requirements.txt` | Used once to fetch the MSMT17-pretrained checkpoint the `autocar` ONNX export below was built from, from the official `kaiyangzhou/deep-person-reid` Model Zoo — same one-time, dev-machine-only, offline step as the `torchreid` row above. Excluded from `requirements.txt` alongside `torchreid` for the same reason. |
| `paho-mqtt` | ≥1.6.1,<2.0.0 | `modules/mqtt_bridge` (`--mqtt` only) — publishes `FollowMeCommand` drive commands to a Pi 4 motor controller over MQTT (`connect_async()` + `loop_start()` for auto-reconnect, `will_set()` for the Last Will and Testament stop signal). Pinned below 2.0 specifically to keep its 1.x `on_connect`/`on_disconnect` callback signature — 2.x requires an explicit `callback_api_version` choice this module doesn't need. Lazily imported only when `--mqtt` is passed, so it's never a hard dependency for a plain `followme` run. |
...
| **YOLOv8n-pose** (`yolov8n-pose.pt`, ultralytics) | PyTorch, via `ultralytics.YOLO` (their `detector/yolov8_pose_torch.py`) | `modules/autocar` (vendored, driven by `autocar_adapter.py`) | Person detection + full-body pose keypoints in one pass — a SEPARATE weights file/instance from this project's own `models/yolo11n.onnx` above. Auto-downloads to the REPO ROOT (not `models/`) on first use — `modules/autocar/config.py`'s `POSE_MODEL_PATH = "yolov8n-pose.pt"` is a bare, process-cwd-relative filename hardcoded inside vendored, byte-for-byte-unmodifiable code, so it can't be relocated into `models/` the way `yolo11n.onnx` was (see [`architecture.md`](architecture.md)'s "A note on model file locations"). |
