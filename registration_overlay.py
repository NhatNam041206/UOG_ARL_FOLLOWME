...
Capture itself still runs no IDENTITY detection (see register_person.py's module docstring) — no
face detection, no embeddings computed here. It does now run one lightweight thing during the
capture states: a live person-COUNT check (registration_data.LiveSubjectDetector), whose result
these functions use to color the ROI box green (exactly 1 person — accepted) or yellow (0 or 2+ —
rejected) and print a status line. draw_capture() takes that count as a plain int; it still does
no detection itself.
"""
...
_ROI_COLOR = (0, 220, 220)  # default/rejected — yellow
_ACCEPT_COLOR = (0, 200, 0)  # exactly 1 person in the ROI — green
...
def draw_roi(image: np.ndarray, roi_percent: RoiPercent, color=_ROI_COLOR) -> RoiPx:
    ...
    cv2.rectangle(image, roi_px[:2], roi_px[2:], color, 2)
    return roi_px
...
def draw_capture(frame: np.ndarray, roi_percent: RoiPercent, instruction: str,
                  saved: int, needed: int, person_count: int) -> np.ndarray:
    """`person_count` is how many people register_person.py's LiveSubjectDetector found with a
    bbox center inside the ROI this tick (0, 1, or 2+) — exactly 1 draws the box green and is what
    register_person.py treats as an acceptable frame to save; 0 or 2+ draws it yellow, and
    register_person.py skips saving until the count is back to exactly 1."""
    display = frame.copy()
    accepted = person_count == 1
    color = _ACCEPT_COLOR if accepted else _ROI_COLOR
    draw_roi(display, roi_percent, color)
    if person_count == 0:
        status = "0 people in frame - REJECTED"
    elif person_count == 1:
        status = "1 person in frame - ACCEPTED"
    else:
        status = f"{person_count} people in frame - REJECTED (need exactly 1)"
    _banner(display, [instruction, f"{saved}/{needed}", status], color)
    return display
